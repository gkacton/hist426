...
# Use `leaflet` to make a map of named places
...
This map reveals a **huge** number of geocoding mistakes -- "DC" and "Armenia" have both ended up in Colombia...
Next I should try using historical gazetteers to geocode rather than modern geocoding tools! 
I can also use either the Getty TGN or another similar dataset to categorize the places mentioned into continental groupings. 
...
The `all_places` dataframe contains all places named in tossup questions, as recognized by SpaCy **and** the Getty Vocabularies. 
...
# Examining collocates from AntConc

```{r 4-grams}
wa_4gram <- read_tsv("data/4-gram_WA.txt")
wm_4gram <- read_tsv("data/4-gram_WM.txt")
nw_4gram <- read_tsv("data/4-gram_NW.txt")
```

```{r most-common}
top_words_wa <- read_tsv("data/top_words_WA_stops.txt")
top_words_wm <- read_tsv("data/top_words_WM_stops.txt")
top_words_nw <- read_tsv("data/top_words_NW_stops.txt")

# distinctive words
keywords_WA <- read_tsv("data/1_keywords_WA.txt")
keywords_WM <- read_tsv("data/1_keywords_WM.txt")
```
...
nhbb_people <- nhbb_clean %>% 
  filter(ID %in% person_IDs == T)

# write txt files
for (j in 1:nrow(nhbb_people)) {
  q_filepath <- paste("people_all/", nhbb_people$ID[j], "_q.txt", sep = "")
  nhbb_people$q_file[j] <- q_filepath
  a_filepath <- paste("people_all/", nhbb_people$ID[j], "_a.txt", sep = "")
  ...
  if(nhbb_people$bonus_q_text[j] != ""){
    bonus_q_filepath <- paste("people_all/", nhbb_people$ID[j], "_bonus_q.txt", sep = "")
    nhbb_people$bonus_q_file[j] <- bonus_q_filepath
    bonus_a_filepath <- paste("people_all/", nhbb_people$ID[j], "_bonus_a.txt", sep = "")
    nhbb_people$bonus_a_file[j] <- bonus_a_filepatx h
...
for(i in 1:nrow(nhbb_people)){
  if(nhbb_people$ID[i] %in% woman_topic == F){
      q_filepath <- paste("people_men/", nhbb_people$ID[i], "_q.txt", sep = "")
      nhbb_people$q_file[i] <- q_filepath
      a_filepath <- paste("people_men/", nhbb_people$ID[i], "_a.txt", sep = "")
  ...
  # write additional files for bonus questions + answers
    if(nhbb_people$bonus_q_text[i] != ""){
      bonus_q_filepath <- paste("people_men/", nhbb_people$ID[i], "_bonus_q.txt", sep = "")
      nhbb_people$bonus_q_file[i] <- bonus_q_filepath
      bonus_a_filepath <- paste("people_men/", nhbb_people$ID[i], "_bonus_a.txt", sep = "")
...
## MESSED UP: ALL WOMEN ARE CURRENTLY COUNTING TWICE IF THEY GOT PICKED UP CORRECTLY BY SPACY
## FIGURE OUT WHICH ARE DUPLICATES AND IF YOU ACTUALLY NEED TO JOIN THE WOMEN DF 

combined_people_count <- full_join(people_count, person_answers_count, by = c("people" = "a_text")) %>% 
  full_join(women_counts, by = c("people" = "names")) %>% 
  full_join(women_answer_counts, by = c("people" = "answer")) %>% 
  replace(is.na(.), 0) %>% 
  mutate(n.total = rowSums(across(where(is.numeric)))) %>% 
  select(people, n.total) %>% 
  arrange(desc(n.total))

head(combined_people_count)
```

So, what are we expecting high schoolers to know? Jesus and the founding fathers. 
